...
async def mute_user(client: Client, message: Message):
    ...
    try:
        success = await mute(client, chat_id, user_id)
        if success:
            await message.reply("🔇 User has been muted.")
        else:
            await message.reply("⚠️ Failed to mute the user.")
    except Exception as e:
        await message.reply(f"❌ Error while muting: {e}")
...
async def unmute_user(client: Client, message: Message):
    ...
    chat_id = message.chat.id
    try:
        success = await unmute(client, chat_id, user_id)
        if success:
